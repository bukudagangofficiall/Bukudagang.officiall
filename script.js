...
// Pengaturan metode pembayaran — Cash selalu aktif (tidak bisa dimatikan), QRIS & Online (Shopee/
// Gofood/Grabfood) bisa dinyalakan/dimatikan admin sendiri lewat menu "Pembayaran".
let paymentSettings = { qrisEnabled: true, onlineEnabled: false, qrisImageUrl: '' };
...
function subscribeAdminScopedData(adminId) {
    ...
    onSnapshot(a('config', 'paymentSettings'), (snap) => {
        ...
        paymentSettings.qrisEnabled = data.qrisEnabled !== false; // default true kalau belum pernah diatur
...
function applyPaymentSettingsToUI() {
    ...
    if (btnQr) btnQr.classList.toggle('hidden', !paymentSettings.qrisEnabled);

    // Online (Shopee/Gofood/Grabfood) butuh DUA syarat: admin nyalain togglenya DAN toko sudah
    // berlangganan. Kalau salah satu tidak terpenuhi, tombolnya disembunyikan dari checkout.
    const btnOnline = document.getElementById('btn-sf');
    if (btnOnline) btnOnline.classList.toggle('hidden', !(paymentSettings.onlineEnabled && isAdminSubscribed));

    // Toggle visual di menu Pembayaran (Admin Panel)
    const toggleQris = document.getElementById('toggle-qris');
    if (toggleQris) {
        toggleQris.classList.toggle('bg-emerald-500', paymentSettings.qrisEnabled);
        toggleQris.classList.toggle('border-2', !paymentSettings.qrisEnabled);
        toggleQris.classList.toggle('border-slate-300', !paymentSettings.qrisEnabled);
        toggleQris.querySelector('i').classList.toggle('hidden', !paymentSettings.qrisEnabled);
    }
    const toggleOnline = document.getElementById('toggle-online');
    const onlineCard = document.getElementById('payment-online-card');
    const onlineSublabel = document.getElementById('payment-online-sublabel');
    if (toggleOnline) {
        const [checkIcon, lockIcon] = toggleOnline.querySelectorAll('i');
        if (!isAdminSubscribed) {
            toggleOnline.className = 'w-6 h-6 rounded-full bg-slate-200 flex items-center justify-center shrink-0';
            checkIcon.classList.add('hidden');
            lockIcon.classList.remove('hidden');
        } else {
            toggleOnline.className = paymentSettings.onlineEnabled
            ...
            checkIcon.classList.toggle('hidden', !paymentSettings.onlineEnabled);
            lockIcon.classList.add('hidden');
        }
    }
    if (onlineCard) onlineCard.classList.toggle('opacity-50', !isAdminSubscribed);
    if (onlineSublabel) onlineSublabel.innerText = isAdminSubscribed ? 'Tap untuk nyalakan/matikan' : 'Butuh langganan aktif';
    lucide.createIcons();
}

// Toggle QRIS/Online dari menu Pembayaran di Admin Panel. Cash sengaja tidak lewat sini sama
// sekali (selalu aktif, tombolnya di-disable di HTML).
function togglePaymentMethod(method) {
    if (method === 'online' && !isAdminSubscribed) {
        return alert('Pembayaran Online (Shopee/Gofood/Grabfood) butuh langganan aktif dulu.');
    }
    if (!window.FB || !window.FB.ready) return showToast('Belum terhubung ke database. Coba lagi sebentar.', 'warn');
    const { db, doc, setDoc } = window.FB;
    const field = method === 'qris' ? 'qrisEnabled' : 'onlineEnabled';
    const newValue = method === 'qris' ? !paymentSettings.qrisEnabled : !paymentSettings.onlineEnabled;
...
function applySubscriptionGates() {
    ['jadwal-kerja', 'riwayat-absen', 'penjualan-produk', 'cashrek'].forEach((key) => {
