...
// Upload foto QRIS baru dari Admin Panel — disimpan di Firebase Storage (bukan file di GitHub
// lagi), URL hasil upload-nya disimpan di Firestore supaya langsung sinkron real-time ke semua
// HP kasir. Ukuran file dibatasi 2MB biar upload tidak lambat di koneksi HP yang pas-pasan.
function uploadQrisImage(fileInput) {
    const file = fileInput.files && fileInput.files[0];
    if (!file) return;
    if (file.size > 2 * 1024 * 1024) {
        fileInput.value = '';
        return alert('Ukuran foto maksimal 2MB. Coba kompres/screenshot ulang foto QRIS-nya.');
    }
    if (!window.FB || !window.FB.ready) return showToast('Belum terhubung ke database. Coba lagi sebentar.', 'warn');

    const adminId = getActiveAdminId();
    const { storage, storageRef, uploadBytes, getDownloadURL, db, doc, setDoc } = window.FB;
    const statusEl = document.getElementById('qris-upload-status');
    if (statusEl) { statusEl.innerText = 'Mengupload...'; statusEl.classList.remove('hidden'); }

    const fileRef = storageRef(storage, `admins/${adminId}/qris.jpg`);
    uploadBytes(fileRef, file)
        .then(() => getDownloadURL(fileRef))
        .then((url) => setDoc(doc(db, ...adminPathSegments('config', 'paymentSettings')), { qrisImageUrl: url }, { merge: true }))
        .then(() => {
        ...
        .catch((err) => {
            ...
            if (statusEl) statusEl.classList.add('hidden');
            showToast('Gagal upload foto. Coba lagi.', 'warn');
        });
    fileInput.value = '';
}
...
function generateAttendanceQR() {
    ...
    if (!window.FB || !window.FB.ready) return showToast('Belum terhubung ke database.', 'warn');
    const { db, doc, setDoc } = window.FB;
    setDoc(doc(db, ...adminPathSegments('config', 'attendanceQrToken')), { token }).then(() => {
