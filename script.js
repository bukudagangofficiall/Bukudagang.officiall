...
let STORE_PHONE = ""; // Nomor HP toko — diisi lewat menu "Profile Toko" di Admin Panel
let STORE_EMAIL = ""; // Email toko — diisi lewat menu "Profile Toko" di Admin Panel

// Pengaturan metode pembayaran — Cash & Nontunai gratis (bisa dinyalakan/dimatikan admin bebas),
// QRIS & Online (Shopee/Gofood/Grabfood) SAMA-SAMA butuh langganan aktif dulu baru bisa dinyalakan.
let paymentSettings = { cashEnabled: true, nontunaiEnabled: false, qrisEnabled: false, onlineEnabled: false, qrisImageUrl: '' };

// --- MODAL ALERT KUSTOM ---
// Ganti alert() bawaan browser (kotak abu-abu jelek, nampilin nama domain "xxx.github.io
// menyatakan") dengan modal bertema sama seperti aplikasi (putih+biru, tanpa judul domain).
// window.alert DITIMPA di sini, jadi SEMUA pemanggilan alert(...) yang sudah ada di seluruh
// file ini otomatis lewat modal baru ini — tidak perlu cari-ganti satu-satu.
function showAppAlert(message) {
    const backdrop = document.getElementById('app-alert-backdrop');
    const msgEl = document.getElementById('app-alert-message');
    if (!backdrop || !msgEl) return; // fallback aman kalau markupnya somehow belum ke-load
...
// Ubah nomor HP format apapun (08xx, 8xx, +62xx, 62xx) jadi format wa.me yang valid (62xx,
// tanpa spasi/tanda apapun). Return null kalau hasilnya kosong/kurang dari 8 digit (dianggap
// bukan nomor valid, supaya tidak menghasilkan link wa.me yang rusak).
...
// Nomor WA tujuan bukti pembayaran QRIS — UTAMAKAN nomor HP admin/toko yang aktif di device ini
// (diisi lewat Profile Toko), supaya tiap toko terima bukti pembayarannya sendiri-sendiri, bukan
// semua masuk ke satu nomor yang sama. ADMIN_WA_NUMBER cuma jadi fallback kalau admin toko itu
// belum sempat isi Nomor HP di Profile Toko-nya.
...
const ADMIN_WA_NUMBER = "62895345452412"; // fallback SAJA — dipakai hanya kalau admin toko belum isi Nomor HP di Profile Toko (lihat getPaymentProofWaNumber())
...
// --- BATASAN LANGGANAN ---
// Status ini dibaca dari field `subscribed` (boolean) di dokumen admins/{adminId} milik admin
// yang aktif di device ini. TIDAK ADA cara bayar/aktivasi otomatis sama sekali sekarang (menu
// "Langganan Saya" masih placeholder "Coming Soon") — satu-satunya cara mengubah status ini
// untuk sekarang adalah edit manual field `subscribed` jadi true di Firebase Console, pada
// dokumen admins/{uid} milik akun yang mau dites.
...
function subscribeAdminScopedData(adminId) {
    ...
    // --- Status Langganan (dokumen PROFIL admin itu sendiri, bukan di bawah config/) ---
    ...
    // --- Pengaturan Pembayaran (Cash selalu aktif, QRIS & Online bisa dinyalakan/dimatikan admin) ---
    onSnapshot(a('config', 'paymentSettings'), (snap) => {
        trackPendingWrites(snap);
        const data = snap.exists() ? snap.data() : {};
        paymentSettings.cashEnabled = data.cashEnabled !== false; // default true kalau belum pernah diatur
        paymentSettings.nontunaiEnabled = data.nontunaiEnabled === true; // default false
        paymentSettings.qrisEnabled = data.qrisEnabled === true; // default false
        paymentSettings.onlineEnabled = data.onlineEnabled === true; // default false
    ...
    // --- Profile Toko (nama toko, nomor HP, email — dinamis) ---
    ...
    // --- Salt QR Absen (dibuat SEKALI per toko, dipakai untuk hitung token harian yang otomatis
    //     berganti sendiri tiap tanggal berubah — lihat getQrToken()) ---
...
// --- PENGATURAN PEMBAYARAN (Cash/QRIS/Online) ---
// Terapkan pengaturan admin ke layar checkout: sembunyikan tombol metode yang dimatikan,
// dan pasang foto QRIS yang di-upload admin (kalau ada) menggantikan file statis qris.jpg.
function applyPaymentSettingsToUI() {
    ...
    // Definisi 4 metode: mana yang butuh langganan, dan pasangan id togglenya di menu Pembayaran
    // + id tombolnya di layar checkout kasir.
    const methods = [
        { key: 'cash', enabled: paymentSettings.cashEnabled, needsSub: false, toggleId: 'toggle-cash', btnId: 'btn-cash', cardId: null, sublabelId: null },
    ...
    methods.forEach(({ enabled, needsSub, toggleId, btnId, cardId, sublabelId }) => {
        // Tombol di layar checkout kasir — cuma tampil kalau nyala DAN (kalau perlu) sudah berlangganan
        const btn = document.getElementById(btnId);
        if (btn) btn.classList.toggle('hidden', !(enabled && (!needsSub || isAdminSubscribed)));

        // Toggle bulat di menu Pembayaran (Admin Panel)
        const toggleEl = document.getElementById(toggleId);
        if (!toggleEl) return;
        const icons = toggleEl.querySelectorAll('i'); // [check] untuk cash/nontunai, [check, lock] untuk qris/online
        ...
        if (cardId) {
            ...
            if (cardEl) {
                cardEl.classList.toggle('opacity-50', locked);
                cardEl.classList.toggle('grayscale', locked); // redupkan warna ikon+badge juga, bukan cuma opacity
                cardEl.classList.toggle('pointer-events-none', locked && cardId !== 'payment-qris-card'); // qris tetap bisa upload/hapus foto walau terkunci
            }
        }
        if (sublabelId) {
    ...
    // Thumbnail preview foto QRIS: tap untuk hapus foto yang sudah diupload (hanya kalau memang
    // sudah ada foto tersimpan, biar tidak nampak bisa ditap padahal masih pakai gambar placeholder).
    const qrisThumbBtn = document.getElementById('qris-preview-thumb');
    if (qrisThumbBtn) {
        qrisThumbBtn.classList.toggle('cursor-pointer', !!paymentSettings.qrisImageUrl);
        qrisThumbBtn.title = paymentSettings.qrisImageUrl ? 'Tap untuk hapus foto QRIS' : '';
    }
    lucide.createIcons();
}

// Toggle metode pembayaran dari menu Pembayaran di Admin Panel. Cash & Nontunai bebas kapan saja,
// QRIS & Online (Shopee/Gofood/Grabfood) WAJIB langganan aktif dulu baru bisa dinyalakan.
function togglePaymentMethod(method) {
    ...
    if (!window.FB || !window.FB.ready) return showToast('Belum terhubung ke database. Coba lagi sebentar.', 'warn');
    const { db, doc, setDoc } = window.FB;
    const fieldMap = { cash: 'cashEnabled', nontunai: 'nontunaiEnabled', qris: 'qrisEnabled', online: 'onlineEnabled' };
    const currentMap = { cash: paymentSettings.cashEnabled, nontunai: paymentSettings.nontunaiEnabled, qris: paymentSettings.qrisEnabled, online: paymentSettings.onlineEnabled };
    const field = fieldMap[method];
    const previousValue = currentMap[method];
    const newValue = !previousValue;

    // UPDATE OPTIMISTIS: langsung ubah tampilan toggle SEBELUM konfirmasi server datang, supaya
    // tombol selalu terasa responsif walau koneksi lambat. Kalau ternyata gagal disimpan (mis.
    // ditolak oleh Firestore Security Rules), tampilan dikembalikan lagi ke posisi semula + alert
    // jelas ke admin (bukan cuma toast kecil yang gampang kelewat).
    paymentSettings[field] = newValue;
    applyPaymentSettingsToUI();

    setDoc(doc(db, ...adminPathSegments('config', 'paymentSettings')), { [field]: newValue }, { merge: true }).catch((err) => {
        console.error(`Gagal ubah pengaturan pembayaran (${field}):`, err);
        paymentSettings[field] = previousValue; // rollback tampilan
        applyPaymentSettingsToUI();
        if (err && err.code === 'permission-denied') {
            alert(`Gagal menyimpan metode pembayaran. Server menolak perubahan field "${field}" (permission-denied). Kemungkinan besar Firestore Security Rules belum mengizinkan field ini — cek aturan untuk dokumen config/paymentSettings.`);
        } else {
            showToast('Gagal menyimpan ke server. Coba lagi.', 'warn');
        }
    });
}

// Hapus foto QRIS yang sudah diupload — dipicu dengan tap langsung di thumbnail preview-nya
// di menu Admin > Pembayaran. Minta konfirmasi dulu karena ini menghapus permanen dari server.
function deleteQrisImage() {
    if (!paymentSettings.qrisImageUrl) return; // belum ada foto, tidak ada yang perlu dihapus
    if (!confirm('Hapus foto QRIS yang sudah diupload?')) return;
    if (!window.FB || !window.FB.ready) return showToast('Belum terhubung ke database. Coba lagi sebentar.', 'warn');
    const { db, doc, setDoc } = window.FB;
    setDoc(doc(db, ...adminPathSegments('config', 'paymentSettings')), { qrisImageUrl: '' }, { merge: true })
        .then(() => showToast('Foto QRIS dihapus.', 'success'))
        .catch((err) => {
            console.error('Gagal hapus foto QRIS:', err);
            showToast('Gagal menghapus foto. Coba lagi.', 'warn');
        });
}

// Kompres foto di BROWSER dulu (resize ke maksimal 500px sisi terpanjang + turunkan kualitas
// JPEG bertahap) sebelum disimpan sebagai base64 langsung di Firestore. Ini dipakai supaya tidak
// perlu Firebase Storage (yang sekarang wajib paket Blaze) — cukup pakai Firestore yang gratis,
// tapi berarti hasilnya wajib dijaga di bawah ~700KB (batas dokumen Firestore itu sendiri 1MB).
function compressImageToDataUrl(file, maxSize = 500) {
    return new Promise((resolve, reject) => {
        ...
        img.onload = () => {
            ...
            // Turunkan kualitas bertahap sampai ukurannya aman buat disimpan di 1 dokumen Firestore.
            let quality = 0.8;
...
// Upload foto QRIS baru dari Admin Panel — disimpan sebagai base64 LANGSUNG di Firestore (bukan
// Firebase Storage), supaya tetap 100% gratis tanpa perlu upgrade paket. URL-nya (data URI)
// langsung sinkron real-time ke semua HP kasir lewat listener paymentSettings yang sudah ada.
...
// Dipanggil tiap kali kasir mengetik di kolom NIK — begitu 6 digit lengkap, langsung dicocokkan
// ke employeesCache (sudah ter-scope ke admin/toko aktif di device ini) dan nama pemiliknya
// ditampilkan sebagai konfirmasi visual SEBELUM kasir lanjut ke PIN.
...
// NIK 6 digit dibuat OTOMATIS (bukan diketik admin), unik dalam ruang lingkup admin ini saja —
// dua admin/toko berbeda boleh punya karyawan dengan angka NIK yang sama, karena employeesCache
// yang dipakai untuk cek keunikan ini sudah ter-scope per-admin (lihat subscribeAdminScopedData).
...
// PIN 4 digit dipakai buat masuk PIN setelah NIK dikenali (layar kunci NIK+PIN). Disimpan apa
// adanya di data karyawan — cukup buat mencegah orang asal pakai NIK teman kerja, bukan pengganti
// keamanan tingkat bank (perangkatnya toh fisik dipegang langsung oleh tim di toko).
...
// Sama seperti applyStoreName() — isi field Nomor HP & Email kalau lagi tidak sedang diketik.
...
function removeScheduleCellJamMasuk() {
    ...
    const savePromise = remaining.length > 0
        ...
        : deleteDoc(doc(db, ...adminPathSegments('schedule', dateStr))); // kosongkan doc kalau tidak ada karyawan lagi, biar rapi di database
...
// Token QR sekarang OTOMATIS berganti sendiri tiap hari (tidak perlu tombol "Generate QR Baru"
// lagi) — dihitung dari kombinasi salt rahasia toko (dibuat sekali, tersimpan di Firestore) +
// tanggal hari ini. Begitu tanggal berganti, token ikut berubah otomatis di semua HP (admin &
// kasir) tanpa perlu sinkron/tulis apapun ke server, karena dihitung ulang tiap kali dibutuhkan.
...
function getQrToken() {
    ...
    const todayStr = getTodayDateStr(); // format YYYY-MM-DD, sudah ada helper-nya di file ini
    return `ABSEN-${todayStr.replace(/-/g, '')}-${hashSaltAndDate(cachedQrSalt + todayStr)}`;
}

// Dipanggil otomatis kalau toko ini belum pernah punya salt sama sekali (baru daftar). Dijaga
// cuma jalan dari konteks Admin Panel (bukan device kasir) supaya tidak race/tertulis berkali-kali.
...
async function renderAttendanceQR() {
    ...
    if (!container) return;
    container.innerHTML = '';
    ensureQrSaltExists(); // kalau toko ini belum pernah punya salt sama sekali, buat sekali otomatis
...
// Setiap orang boleh mendaftar akun admin sendiri (TIDAK lagi dikunci cuma 1x seumur aplikasi).
// Tab Daftar & Masuk SELALU ditampilkan berdua; default buka di tab Masuk.
...
// Pendaftaran admin TERBUKA — siapa saja boleh daftar akun admin sendiri, dengan stok/produk/
// karyawan yang otomatis kosong (belum ada apapun) sampai admin ini isi sendiri. Begitu berhasil
// daftar, device ini langsung "terikat" ke toko admin baru ini lewat switchActiveAdminAndResync().
async function registerFirstAdmin() {
    ...
    try {
        ...
        // Profil admin baru: dokumen ini sekadar identitas pemilik toko, stok/karyawan tetap
        // kosong sampai diisi karena disimpan terpisah di admins/{uid}/config/... (lihat
        // subscribeAdminScopedData) yang belum ada dokumennya sama sekali untuk uid baru ini.
        await setDoc(doc(db, 'admins', cred.user.uid), {
            email,
            createdAt: serverTimestamp()
        });
        // Nama Toko & No. HP yang diisi saat daftar langsung disimpan sebagai storeProfile —
        // sama persis dokumen yang dipakai menu "Profile Toko", jadi nanti bisa diubah lagi
        // kapan saja dari sana tanpa perlu isi ulang dari awal.
...
async function checkLogin() {
    ...
    try {
        ...
        // Kalau device ini sebelumnya belum pernah dipakai, atau dipakai admin/toko LAIN,
        // sambungkan ulang device ini ke stok & karyawan milik admin yang baru login ini.
...
// Versi "paksa buka" (bukan toggle) — dipakai saat pindah section lewat sidebar, supaya isi
// menu langsung kelihatan tanpa user harus tap accordion-nya lagi sekali lagi secara manual.
...
// Versi "paksa buka" untuk kategori (Produk/Karyawan/Laporan) — sama alasannya seperti
// openAdminAccordion() di atas.
...
function showPage(page) {
    ...
    if (enteringAdmin) {
        ...
        openAdminSidebar(); // sapa admin dengan menu dulu, bukan langsung tampilan penuh
    } else {
        closeAdminSidebar();
...
// --- SIDEBAR MENU ADMIN ---
// Satu-satunya tombol admin di header utama (ikon 3 garis) — perilakunya beda tergantung posisi:
// - Belum masuk Admin Panel: jadi pintu masuk Login Admin (perilaku ikon gembok yang lama).
// - Sudah di dalam Admin Panel: jadi tombol buka/tutup sidebar menu, tidak buka Login lagi.
...
// Kunci/buka 4 bagian yang WAJIB langganan (tidak ada versi gratisnya sama sekali, beda dari
// batas Produk/Kategori/Karyawan yang cuma dibatasi jumlahnya). Dipanggil tiap kali status
// langganan berubah, dan juga tiap kali Admin Panel dibuka.
function applySubscriptionGates() {
    ['jadwal-kerja', 'riwayat-absen', 'penjualan-produk', 'cashrek', 'stock'].forEach((key) => {
        ...
        // Redupkan baris menunya sendiri (sebelum di-tap) + tampilkan ikon gembok menggantikan
        // panah, supaya kelihatan jelas menu ini belum bisa dipakai tanpa perlu tap dulu.
    ...
    applyPaymentSettingsToUI(); // status Online (butuh langganan) ikut berubah tiap status ini update
    lucide.createIcons();
}

// Tombol "Langganan Sekarang" — belum ada alur pembayaran apapun, cukup kasih tahu masih
// coming soon lewat modal yang sama dipakai untuk peringatan lain di seluruh aplikasi.
...
// Pindah ke 1 section tertentu (Profile Toko/Produk/Karyawan/Laporan/Langganan) — cuma section
// itu yang ditampilkan, sisanya disembunyikan, mirip tab. Isi menunya langsung dibuka penuh
// (bukan cuma judul kategori doang) supaya tidak perlu tap 2x. Drawer otomatis tertutup setelahnya.
function showAdminSection(sectionKey) {
    ...
    // Langsung buka isi menunya, bukan cuma tampilkan judul kategori yang masih perlu di-tap lagi.
...
// Tombol "Keluar" di sidebar — beda dari sekadar tutup drawer (closeAdminSidebar), ini benar-benar
// keluar dari Admin Panel dan balik ke tampilan Kasir.
