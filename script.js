...
let isAdminSubscribed = true; // TESTING: paksa true supaya semua fitur berbayar kebuka dulu buat dicoba
...
function subscribeAdminScopedData(adminId) {
    ...
    onSnapshot(doc(db, 'admins', adminId), (snap) => {
        ...
        isAdminSubscribed = true; // TESTING: paksa true, abaikan field 'subscribed' asli dari Firestore
...
function renderAttendanceRequiredToggle() {
    ...
    label.innerText = attendanceRequired ? 'Aktif' : 'Tidak Aktif';
    const dot = btn.querySelector('i');
    if (dot) dot.setAttribute('class', attendanceRequired ? 'w-2.5 h-2.5 text-emerald-500 fill-emerald-500' : 'w-2.5 h-2.5 text-red-500 fill-red-500');
    if (note) note.classList.toggle('hidden', attendanceRequired);
    lucide.createIcons();
}
