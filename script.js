...
// Pengaturan metode pembayaran — Cash SELALU aktif (statis, tidak lewat state ini sama sekali,
// supaya kasir selalu punya minimal 1 metode bayar). Nontunai gratis (bebas nyala/mati admin),
...
function openCheckout() {
    ...
    renderCartItems();
    try {
        updatePaymentButtons(); // jaga-jaga: pastikan tombol QRIS/Online/Nontunai yang belum diaktifkan admin tetap tersembunyi setiap kali Detail Pesanan dibuka
    } catch (err) {
...
function applyPaymentSettingsToUI() {
    ...
    // Blok "Upload Foto QRIS" cuma tampil kalau SUDAH berlangganan — sebelum itu disembunyikan
    // total (bukan cuma diredupkan), supaya tidak ada yang salah kira bisa upload padahal belum bisa.
    ...
    // Definisi metode yang TOGGLE-nya diatur dari sini (Cash TIDAK termasuk — selalu aktif,
    // badge-nya statis langsung di HTML, tidak pernah disembunyikan dari checkout).
    const methods = [
...
// Toggle metode pembayaran dari menu Pembayaran di Admin Panel. Cash TIDAK lewat sini sama sekali
// (selalu aktif, badge-nya statis di HTML). Nontunai bebas kapan saja, QRIS & Online (Shopee/
// Gofood/Grabfood) WAJIB langganan aktif dulu baru bisa dinyalakan.
//
// Update DULUAN di layar (optimistic) sebelum nunggu Firestore — supaya kalau ada masalah kirim
// ke server (misal Security Rules belum izinkan field baru), admin langsung lihat toggle-nya
// balik lagi ke posisi semula + pesan error yang jelas, bukan kelihatan "diam saja tidak ngapa-ngapain".
function togglePaymentMethod(method) {
    ...
    if (!field) return; // 'cash' atau method tidak dikenal -> tidak ada apa-apa yang perlu ditoggle

    const oldValue = paymentSettings[field];
    paymentSettings[field] = !oldValue;
    applyPaymentSettingsToUI(); // langsung kelihatan berubah di layar
    ...
    setDoc(doc(db, ...adminPathSegments('config', 'paymentSettings')), { [field]: paymentSettings[field] }, { merge: true })
        .catch((err) => {
            ...
            paymentSettings[field] = oldValue; // gagal simpan -> balikin lagi tampilannya
            applyPaymentSettingsToUI();
...
function setPayment(method) {
    ...
    // Aktifkan tombol Konfirmasi DULUAN, sebelum render lain — supaya walau ada error tak terduga
    // di kode render tombol (updatePaymentButtons dkk), pemilihan metode bayar kasir tetap dianggap
    // sah dan Konfirmasi tidak pernah ketinggalan mati.
...
function updatePaymentButtons() {
    ...
    // PENTING: baris di atas mengganti className SELURUHNYA (termasuk menghapus class "hidden"
    // kalau ada) — jadi begitu selesai, pasang lagi status tampil/sembunyi sesuai pengaturan admin
    // (Nontunai/QRIS/Online). Tanpa baris ini, metode yang sengaja dimatikan admin akan muncul lagi
    // tiap kali tombol pembayaran di-render ulang.
    applyPaymentSettingsToUI();
